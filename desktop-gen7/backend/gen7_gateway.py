...
<body>
  ...
  <footer class="windi-footer">/* Footer institucional WINDI — SEPARADO do conteúdo */</footer>
...
- NUNCA usar mailto: ou tel: — usar href="#contact" ou formulário interno
- O footer WINDI deve estar FORA de qualquer card/section de conteúdo
- NUNCA incluir "Generated by WINDI", "Receipt:", "Hash:" no conteúdo — isso é injectado automaticamente pelo export engine
...
- Idioma detectado automaticamente (PT/DE/EN)
- NUNCA usar mailto: ou tel: — usar href="#contact" ou placeholder [EMAIL]
- NUNCA perguntar. PRODUZ HTML AGORA."""
        ...
        try:
            ...
            if r.status_code in (200, 201):
                ...
                if _active_motor and _ANTHROPIC_KEY:
                    ...
                    _log.info(f"[MOTOR] {_active_motor} detected, key={_ANTHROPIC_KEY[:20]}...")
                    def _call_motor():
                        _log.info(f"[MOTOR] Starting Claude call...")
                        ...
                        _log.info(f"[MOTOR] Claude responded, length={len(_r.content[0].text)}")
                        return _r.content[0].text
                    draft_message = await _asyncio.to_thread(_call_motor)

                    # ── Sanitize: remove code block wrappers ──────────────
                    import re
                    draft_message = re.sub(r'^```html\s*\n?', '', draft_message)
                    draft_message = re.sub(r'\n?```\s*$', '', draft_message)
                    draft_message = draft_message.strip()
                    _log.info(f"[MOTOR] After sanitize, length={len(draft_message)}")
...
# === Web Export (P3 — Standalone HTML + Ledger Seal) ===
class ExportWebRequest(BaseModel):
    session_id: str
    html_content: str
    sub_type: Optional[str] = "micro_page"  # micro_page, landing, mini_site, doc_web
    title: Optional[str] = "WINDI Document"
    description: Optional[str] = "Verified content by WINDI Publishing House"


# P4B: OG tags for WhatsApp/Social preview
STANDALONE_WRAPPER = """<!DOCTYPE html>
<html lang="de">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <meta name="generator" content="WINDI Web Engine">
    <title>{TITLE}</title>
    <!-- P4B: Open Graph tags for WhatsApp/Social preview -->
    <meta property="og:title" content="{TITLE}">
    <meta property="og:description" content="{DESCRIPTION}">
    <meta property="og:url" content="https://windi-domain.com/sites/{RECEIPT_ID}/">
    <meta property="og:type" content="website">
    <meta property="og:image" content="https://windi-domain.com/verify-public/static/dragon-three-seal.svg">
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .windi-container { max-width: 960px; margin: 0 auto; padding: 2rem; }
        /* WINDI Tokens */
        .windi-gold { color: #8B6914; }
        .windi-dark { color: #2C2924; }
        .windi-bg-klar { background: #F5F0E0; }
        .windi-bg-noir { background: #0E0E14; color: #F5F0E0; }
        /* Responsive */
        @media (max-width: 768px) {
            .windi-container { padding: 1rem; }
        ...
        .windi-seal {
            margin-top: 3rem;
            padding-top: 1.5rem;
            border-top: 1px solid #DDD6C2;
            font-size: 0.75rem;
            color: #6B6560;
            text-align: center;
        }
        .windi-seal a { color: #8B6914; text-decoration: none; }
    </style>
</head>
<body>
<div class="windi-container">
{CONTENT}
</div>
<footer class="windi-seal">
    <p>Generated by WINDI Web Engine · <a href="https://windi-domain.com/verify-public/?id={RECEIPT_ID}" target="_blank">Verify authenticity</a></p>
    <p>Receipt: {RECEIPT_ID} · Hash: {HASH_SHORT}</p>
</footer>
</body>
</html>"""
...
async def export_web_standalone(req: ExportWebRequest):
    """
    Export Canvas HTML as standalone file + Ledger seal.

    1. Wraps content in standalone HTML with embedded assets
    2. Calculates SHA-256 hash
    3. Seals to Forensic Ledger
    4. Returns complete HTML ready for download

    Receipt format per Business Case:
    {
        "doc_type": "web",
        "sub_type": "micro_page|landing|mini_site|doc_web",
        "export_format": "html",
        "standalone": true
    }
    """
    receipt_id = f"WINDI-WEB-{req.session_id}"

    # Calculate hash of original content
    content_hash = hashlib.sha256(req.html_content.encode()).hexdigest()
    hash_short = content_hash[:16]

    # Build standalone HTML with OG tags
    standalone_html = STANDALONE_WRAPPER.replace("{CONTENT}", req.html_content)
    standalone_html = standalone_html.replace("{RECEIPT_ID}", receipt_id)
    standalone_html = standalone_html.replace("{HASH_SHORT}", hash_short)
    standalone_html = standalone_html.replace("{TITLE}", req.title or "WINDI Document")
    standalone_html = standalone_html.replace("{DESCRIPTION}", req.description or "Verified content by WINDI Publishing House")
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            ledger_payload = {
                "id": receipt_id,
                "actor": "gen7-web-export",
                "app": "windi-web-engine",
                "doc_name": f"Web Export {req.session_id}",
                "doc_type": "doc",  # Ledger requires standard types
                "governance_level": "HIGH",
            ...
            ledger_ok = r.status_code in (200, 201)
        except Exception:
            ledger_ok = False
    ...
    return {
        ...
        "sub_type": req.sub_type,
        "standalone": True,
        "ledger_sealed": ledger_ok,
        "verify_url": f"https://windi-domain.com/verify-public/?id={receipt_id}"
    }


# === P4A: Web Publish (WINDI Hosting) ===
MICROSITES_DIR = Path("/opt/windi/microsites")


class PublishWebRequest(BaseModel):
    session_id: str
    html_content: str
    title: Optional[str] = "WINDI Document"
    description: Optional[str] = "Verified content by WINDI Publishing House"
    sub_type: Optional[str] = "micro_page"


@app.post("/api/publish/web")
async def publish_web_hosting(req: PublishWebRequest):
    """
    P4A: Publish HTML to WINDI Hosting (/sites/{receipt_id}/)

    1. Generates standalone HTML with OG tags
    2. Saves to /opt/windi/microsites/{receipt_id}/index.html
    3. Seals to Forensic Ledger with hosted_at metadata
    4. Returns hosted URL + short URL

    WhatsApp ready: OG tags + short URL for sharing
    """
    receipt_id = f"WINDI-WEB-{req.session_id}"
    site_dir = MICROSITES_DIR / receipt_id
    ...
    # URLs
    hosted_url = f"https://windi-domain.com/sites/{receipt_id}/"
    short_url = f"https://windi-domain.com/s/{receipt_id}"
    verify_url = f"https://windi-domain.com/verify-public/?id={receipt_id}"
    ...
    async with httpx.AsyncClient(timeout=5.0) as client:
        try:
            ledger_payload = {
                "id": receipt_id,
                "actor": "gen7-web-publish",
                "app": "windi-hosting",
                "doc_name": req.title or f"Web {req.session_id}",
                "doc_type": "doc",
                "governance_level": "HIGH",
                "content_hash": content_hash,
                "sge_score": 1.0,
                "metadata": {
                    ...
                    "export_format": "html",
                    "standalone": True,
                    "hosted_at": hosted_url,
                    "short_url": short_url,
                    "og_title": req.title,
                    "og_description": req.description,
                    "whatsapp_ready": True,
    ...
    wa_url = f"https://wa.me/?text={wa_text.replace(' ', '%20').replace(chr(10), '%0A')}"
