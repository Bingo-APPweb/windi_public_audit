#!/usr/bin/env python3
"""
A4 Desk BABEL - Tiptap Edition v4.6
...
def health():
    return jsonify({"status": "healthy", "service": "A4 Desk BABEL v4.6", "version": "4.6", "compliance": ["EU AI Act", "BSI", "DSGVO"]})
...
# ============================================
# v4.6 NEW: Profile Update Endpoint
# ============================================
...
def verify_receipt(receipt_id):
    ...
    conn.close()
    if row:
        return jsonify({"verified": True, "document_id": row["id"], "title": row["title"], "status": row["status"], "receipt": json.loads(row["receipt"]) if row["receipt"] else {}, "principle": "KI verarbeitet. Mensch entscheidet. WINDI garantiert."})
...
def export_document(doc_id, fmt):
    ...
    html_content = f'''<!DOCTYPE html><html><head><meta charset="UTF-8"><title>{title}</title><style>body{{font-family:Helvetica,Arial,sans-serif;font-size:11pt;line-height:1.6;margin:25mm 20mm}}h1{{font-size:18pt;color:#1a365d;border-bottom:2px solid #3182ce;padding-bottom:8pt}}.footer{{margin-top:40pt;border-top:2pt solid #0d9488;padding-top:15pt}}</style></head><body><h1>{title}</h1><div>{content_html}</div>{human_authorship_notice if "HUMAN AUTHORSHIP" not in content_html and "menschlichen Autoren" not in content_html else ""}<div class="footer"><div style="font-size:10pt;color:#0d9488;font-weight:bold">WINDI Publishing House</div><div style="font-size:8pt">WINDI-QUITTUNG: {receipt_id} | Hash: {receipt_hash}</div><div style="font-size:8pt">Autor: {author_info.get("name","-")} ({author_info.get("employee_id","-")}) | Prüfer: {witness_info.get("name","-")} ({witness_info.get("employee_id","-")})</div><img src="data:image/png;base64,{qr_base64}" style="width:60px;margin-top:10px"/><div style="font-size:7pt;color:#718096;margin-top:5px">A4 Desk BABEL v4.6 | EU AI Act Compliant</div></div></body></html>'''
...
def chat():
    ...
    try:
        resp = requests.post(f"{CONFIG['gateway']}/api/chat", json={"message": message, "context": context, "dragon": dragon, "lang": data.get("lang", "de")}, timeout=60)
