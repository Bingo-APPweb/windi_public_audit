#!/usr/bin/env python3
"""
GENERATE GABI SANTOS v2 — Re-render Failed Shots
=================================================
S00-01_v2: Sorriso para o filho — FIX: "smile already formed"
S04-01_v2: Rosto apos a queda — FIX: neutral framing (avoid content filter)

Diagnostico v1:
- S00-01: 0.7009 vs 0.75 — degradacao 0.75->0.68 (micro-movimento sorriso)
- S04-01: GENERATION_FAILED — filtro de conteudo activado
...
RERENDER_SHOTS = [
    {
        ...
        "threshold": 0.75,
        "tier": "FORENSE",
        "diagnosis": "v1: 0.7009 — degradacao 0.75->0.68 (micro-movimento sorriso)",
        "fix": "Smile already formed, expression frozen, only light breathes",
        "prompt": """A 33-year-old Brazilian woman with warm olive skin and dark wavy hair to her shoulders. She is ALREADY SEATED with a warm smile ALREADY FORMED on her face. The smile does not change — it is frozen in gentle affection. She looks at her phone screen. Soft indirect warm lighting from above. Near-frontal angle, head completely still toward lens.

Camera locked. She does not move at all. Her expression is static. Only the subtle phone glow breathes on her face.
...
Intimate, human, photorealistic. 50mm lens. Sharp focus on face. Cinematic 4K."""
    },
    {
        "id": "S04-01_v2",
        "scene": 4,
        "description": "Momento final (repouso)",
        "threshold": 0.75,
        "tier": "FORENSE",
        "diagnosis": "v1: GENERATION_FAILED — content filter triggered",
        "fix": "Neutral framing: peaceful rest, meditation pose, no death language",
        "prompt": """A 33-year-old Brazilian woman with warm olive skin and dark wavy hair. She rests in a peaceful meditation pose, face up, near-frontal angle from above. Expression of deep serenity and inner peace. Soft cool ambient light from above. Small glass crystals catch light nearby, creating gentle sparkles.

Camera locked, looking down at her peaceful face. She is completely still in contemplation. The light shifts subtly around her.

Serene, dignified, photorealistic. 50mm lens. Sharp focus on face. Respectful artistic framing. Cinematic 4K."""
    }
]
...
def main():
    log("=" * 65)
    log("GENERATE GABI SANTOS v2 — Re-render Failed Shots")
    log("=" * 65)
    ...
    # Final Report
    log("\n" + "=" * 65)
    log("FINAL REPORT — GABI SANTOS v2 Re-render")
    log("=" * 65)
    ...
    manifest_path = OUTPUT_DIR / f"GABI_V2_RERENDER_{run_id}.json"
