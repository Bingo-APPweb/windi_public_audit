...
// ═══════════════════════════════════════════════════════
// WINDI API CLIENT — Ponte com Backend Real
// ═══════════════════════════════════════════════════════
...
const windiAPI = {
  ...
  createDoc: () =>
    fetch(`${API}/documents`, { method: 'POST' }).then(r => r.json()),

  getDoc: (id) =>
    fetch(`${API}/documents/${id}`).then(r => r.json()),

  saveDoc: (id, title, content) =>
  ...
  verifyDoc: (id) =>
    fetch(`${API}/documents/${id}/verify`).then(r => r.json()),

  exportDoc: (id, format) => {
    window.open(`${API}/documents/${id}/export/${format}`, '_blank');
  },
  ...
  applyTemplate: (docId, templateId) =>
    fetch(`${API}/documents/${docId}/apply-template`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ template_id: templateId })
    }).then(r => r.json()),

  chat: (message, docId, lang) =>
    fetch(`${API}/chat`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ message, document_id: docId, lang })
    }).then(r => r.json()),
};
    ...
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
        ...
        <button onClick={() => setOpen(false)} style={{
          background: "none", border: "none", color: "#A0A0A0", cursor: "pointer", fontSize: "1.2rem",
        }}>✕</button>
      </div>
      ...
      <div style={{ marginBottom: "16px" }}>
        ...
        />
        <div style={{ display: "flex", justifyContent: "space-between", fontFamily: jm, fontSize: "0.55rem", color: "#8A8A8A" }}>
      ...
      {[
        ...
        >
          ...
          <span style={{
            ...
            background: config[toggle.key] ? "#D4AF37" : "#3A3A3A", position: "relative",
      ...
      <div style={{
        ...
        >
          ...
          <div style={{
            ...
            <span style={{ fontFamily: jm, fontSize: `${0.6 * sz}rem`, color: "#8A8A8A" }}>{t.timerText}</span>
          </div>
          ...
          <div style={{ display: "flex", gap: "10px", justifyContent: "center", flexWrap: "wrap" }}>
            <button onClick={() => { if (reason.trim()) onDecision("approved"); }} style={{
              ...
              borderRadius: "6px", color: reason.trim() ? "#10B981" : "#6B6560",
            ...
            <button onClick={() => { if (reason.trim()) onDecision("overridden"); }} style={{
              ...
              borderRadius: "6px", color: reason.trim() ? "#EF4444" : "#6B6560",
              fontFamily: jm, fontSize: `${0.75 * sz}rem`, cursor: reason.trim() ? "pointer" : "not-allowed",
          ...
          {!reason.trim() && (
            <div style={{ fontFamily: jm, fontSize: `${0.55 * sz}rem`, color: "#8A8A8A", marginTop: "12px" }}>
              * {t.reasonPlaceholder}
            </div>
          )}
        </div>
...
function SovereignEditor({ lang: externalLang, setLang: externalSetLang }) {
  ...
  const [sgeScore, setSgeScore] = useState(null);
  const [riskLevel, setRiskLevel] = useState('R0');
  const [sgeFlags, setSgeFlags] = useState([]);
  ...
  // Auto-create document when mode is selected
  useEffect(() => {
    if (mode && !activeDocId) {
      windiAPI.createDoc().then(res => {
        if (res.id) {
  ...
  useEffect(() => {
    if (!activeDocId || !content || isSealed) return;
    setSaveStatus('saving');
    const timer = setTimeout(() => {
      windiAPI.saveDoc(activeDocId, docTitle || 'Untitled', content)
        .then(res => {
          if (res.saved) setSaveStatus('saved');
          else setSaveStatus('error');
        })
        .catch(() => setSaveStatus('error'));
    }, 2000);
    return () => clearTimeout(timer);
  }, [content, docTitle, activeDocId, isSealed]);

  // SGE real-time scan with debounce
  useEffect(() => {
    if (!activeDocId || !content || content.length < 20 || isSealed) return;
    const timer = setTimeout(() => {
      windiAPI.scanDoc(activeDocId, content)
        .then(res => {
          setAgentState(res.status || 'flow');
          setSgeScore(res.sge_score);
          setRiskLevel(res.risk_level || 'R0');
          setSgeFlags(res.flags || []);
        })
        .catch(() => setAgentState('flow'));
    }, 3000);
    return () => clearTimeout(timer);
  }, [content, activeDocId, isSealed]);
  ...
  const handleSeal = () => {
    if (!activeDocId || isSealed) return;
    windiAPI.sealDoc(activeDocId, 'Sovereign Editor User')
      .then(res => {
        if (res.sealed) {
          setIsSealed(true);
          setAgentState('flow');
          alert('✓ Document sealed!\nHash: ' + res.seal_hash.slice(0, 16) + '...');
        }
      })
      .catch(err => console.error('Seal failed:', err));
  };

  const handleExport = (format) => {
    if (activeDocId) windiAPI.exportDoc(activeDocId, format);
  };
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: "center", maxWidth: "600px", animation: "fadeInUp 1s ease" }}>
          ...
          <div style={{
            fontFamily: of_, fontSize: "0.8rem", fontWeight: 200, color: "#8A8A8A", marginBottom: "48px",
          }}>{t.welcomeSub}</div>
          ...
          <div style={{
            ...
            {[
              ...
              >
                ...
                <div style={{ fontFamily: of_, fontSize: "0.72rem", fontWeight: 300, color: "#A0A0A0", lineHeight: 1.5 }}>
                  {m.desc}
                </div>
              </button>
            ))}
      ...
      <div style={{
        ...
        <div style={{ textAlign: "center", maxWidth: "500px", animation: "fadeInUp 0.8s ease" }}>
          ...
          <button onClick={() => setMode(null)} style={{
            ...
            color: "#8A8A8A", fontFamily: jm, fontSize: "0.65rem", cursor: "pointer",
    ...
    <div style={{
      ...
      <header style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
          ...
          {mode !== "easy" && (
            <span style={{ fontFamily: jm, fontSize: "0.55rem", color: "#9A9A9A" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
          ...
          <button onClick={() => { setMode(null); setDocType(null); }} style={{
            ...
            fontFamily: jm, fontSize: "0.55rem", color: "#8A8A8A",
          }}>
      ...
      <main style={{ flex: 1, display: "flex", justifyContent: "center", padding: `${24 * sz}px` }}>
        <div style={{
          ...
          <div style={{
            ...
            {!isEasy && (
              <div style={{ fontFamily: jm, fontSize: "0.55rem", color: "#9A9A9A" }}>
            ...
            <div style={{ display: "flex", gap: "8px" }}>
              {!isEasy && (
                <>
                  <button style={{
                    padding: `${10 * sz}px ${20 * sz}px`,
                    background: saveStatus === 'saved' ? "rgba(16,185,129,0.1)" : "transparent",
                    border: `1px solid ${saveStatus === 'saved' ? "#10B981" : "rgba(255,255,255,0.08)"}`,
                    borderRadius: "6px",
                    color: saveStatus === 'saved' ? "#10B981" : "#888",
                    fontFamily: jm, fontSize: `${0.7 * sz}rem`, cursor: "pointer",
                  }}>
                    {saveStatus === 'saving' ? '...' : saveStatus === 'saved' ? '✓' : ''} {t.save}
                  </button>
                  <button
              ...
              <button
                onClick={handleSeal}
                disabled={content.length === 0 || isSealed}
                style={{
                padding: `${12 * sz}px ${isEasy ? 40 : 24}px`,
                background: isSealed ? "rgba(16,185,129,0.15)" : content.length > 0 ? "rgba(212,175,55,0.08)" : "transparent",
                border: `1px solid ${isSealed ? "#10B981" : content.length > 0 ? "#D4AF37" : "rgba(255,255,255,0.08)"}`,
                borderRadius: "6px",
                color: isSealed ? "#10B981" : content.length > 0 ? "#D4AF37" : "#6B6560",
                fontFamily: jm, fontSize: `${0.8 * sz}rem`,
                cursor: content.length > 0 && !isSealed ? "pointer" : "default",
                transition: "all 0.3s",
                opacity: isSealed ? 0.7 : 1,
              }}>
                {isSealed ? '✓ Sealed' : `🛡️ ${t.seal}`}
              </button>
            </div>
          </div>
        </div>
