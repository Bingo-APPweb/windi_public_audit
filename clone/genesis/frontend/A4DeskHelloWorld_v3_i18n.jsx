...
function TypeWriter({ text, speed = 40, delay = 0, onDone, gold, style = {} }) {
  ...
  return (
    <span style={{ color: gold ? "#D4AF37" : "#8A8A8A", ...style }}>
      {displayed}
...
function ISPCard({ isp, index, visible }) {
  return (
    <div style={{
      ...
      <span style={{ color: "#555", fontSize: "0.6rem", padding: "1px 4px", background: "rgba(255,255,255,0.03)", borderRadius: "2px" }}>{isp.level}</span>
    </div>
  );
}
    ...
    <div style={{ display: "flex", justifyContent: "center", gap: "4px", marginBottom: "32px" }}>
      {["pt", "de", "en"].map(l => (
        <button key={l} onClick={() => setLang(l)} style={{
          ...
          <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: "0.65rem", color: lang === l ? "#D4AF37" : "#555", letterSpacing: "0.08em", textTransform: "uppercase" }}>{l}</span>
        </button>
      ))}
    </div>
  );
...
export default function A4DeskHelloWorld({ lang: externalLang, setLang: externalSetLang, onComplete }) {
  ...
  const renderInversion = () => {
    ...
    return <>{parts[0]}<span style={{ color: "#D4AF37" }}>{t.separated}</span>{parts[1]}<span style={{ color: "#D4AF37" }}>{t.unites}</span>{parts[2]}</>;
  };

  const pillars = [{ n: "I", ...t.pillar1 }, { n: "II", ...t.pillar2 }, { n: "III", ...t.pillar3 }, { n: "IV", ...t.pillar4 }];
    ...
    <div style={{ minHeight: "100vh", background: "#0A0A0A", color: "#E0E0E0", fontFamily: of_, position: "relative", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: "920px", margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ marginBottom: "40px", textAlign: "center" }}>
          <div style={{ fontSize: "0.65rem", fontFamily: jm, color: "#444", letterSpacing: "0.3em", textTransform: "uppercase", marginBottom: "12px" }}>
          ...
          <div style={{ fontFamily: of_, fontSize: "0.9rem", fontWeight: 200, color: "#666", letterSpacing: "0.15em" }}>{t.tagline}</div>
        </div>
        ...
        {!bootStarted && (
          <div style={{ maxWidth: "720px", margin: "0 auto 48px", animation: "fadeInUp 1.2s ease 0.3s both" }}>
            ...
            <div style={{
              ...
              <div style={{ fontFamily: jm, fontSize: "0.6rem", color: "#555", letterSpacing: "0.4em", textTransform: "uppercase", marginBottom: "20px" }}>{t.sacredCouple}</div>
              ...
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", gap: "clamp(20px, 5vw, 40px)", marginBottom: "20px", flexWrap: "wrap" }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontFamily: bg, fontSize: "clamp(2.5rem, 6vw, 4rem)", fontWeight: 300, color: "#D4AF37", lineHeight: 1, fontStyle: "italic", animation: "breathe 4s ease infinite" }}>a</div>
                  <div style={{ fontFamily: of_, fontSize: "0.75rem", color: "#AAA", fontWeight: 300, marginTop: "4px" }}>{t.soul}</div>
                  <div style={{ fontFamily: jm, fontSize: "0.58rem", color: "#666", marginTop: "4px", fontStyle: "italic" }}>{t.lakshmi}</div>
                </div>
                <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "4px" }}>
                  ...
                  <div style={{ fontFamily: jm, fontSize: "0.5rem", color: "#333", letterSpacing: "0.2em" }}>IA ↔ H</div>
                </div>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontFamily: bg, fontSize: "clamp(2.5rem, 6vw, 4rem)", fontWeight: 800, color: "#D4AF37", lineHeight: 1, animation: "breathe 4s ease 2s infinite" }}>4</div>
                  <div style={{ fontFamily: of_, fontSize: "0.75rem", color: "#AAA", fontWeight: 300, marginTop: "4px" }}>{t.kernel}</div>
                  <div style={{ fontFamily: jm, fontSize: "0.58rem", color: "#666", marginTop: "4px", fontStyle: "italic" }}>{t.narayana}</div>
                </div>
              </div>
              ...
              <div style={{ display: "flex", justifyContent: "center", gap: "12px", flexWrap: "wrap", marginBottom: "16px" }}>
                {[{ a: t.layerAlma, f: t.layer4Pilares, l: t.layerAliance }, { a: t.layerAuthentic, f: t.layer4Dim, l: t.layerShield }, { a: t.layerAlliance2, f: t.layer4Witnesses, l: t.layerPact }].map((x, i) => (
                  <div key={i} style={{ fontFamily: jm, fontSize: "0.58rem", color: "#555", padding: "4px 10px", background: "rgba(0,0,0,0.2)", borderRadius: "3px" }}>
                    <span style={{ color: "#D4AF37" }}>{x.a}</span><span style={{ color: "#333", margin: "0 4px" }}>+</span>
                    <span style={{ color: "#D4AF37" }}>{x.f}</span><span style={{ color: "#333", margin: "0 4px" }}>=</span>
                    <span style={{ color: "#888" }}>{x.l}</span>
                  </div>
                ))}
              </div>
              <div style={{ fontFamily: of_, fontSize: "0.75rem", fontWeight: 200, color: "#777", fontStyle: "italic", lineHeight: 1.6 }}>"{t.altarQuote}"</div>
            </div>
            ...
            <div style={{ padding: "24px", background: "rgba(212,175,55,0.02)", border: "1px solid rgba(212,175,55,0.08)", borderRadius: "6px 6px 0 0", borderBottom: "none" }}>
              <div style={{ fontFamily: bg, fontSize: "0.7rem", fontWeight: 600, color: "#D4AF37", letterSpacing: "0.3em", marginBottom: "16px", textTransform: "uppercase" }}>D . e . s . k .</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "10px" }}>
                {[t.desk_D, t.desk_E, t.desk_S, t.desk_K].map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: "8px", alignItems: "flex-start" }}>
                    <span style={{ fontFamily: bg, fontSize: "1.4rem", fontWeight: 800, color: "#D4AF37", lineHeight: 1, minWidth: "20px" }}>{"DESK"[i]}</span>
                    <div>
                      <div style={{ fontFamily: of_, fontSize: "0.82rem", color: "#BBB", fontWeight: 400 }}>{item.word}</div>
                      <div style={{ fontFamily: jm, fontSize: "0.58rem", color: "#555", marginTop: "2px" }}>{item.desc}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ padding: "24px", background: "rgba(212,175,55,0.03)", border: "1px solid rgba(212,175,55,0.08)", borderRadius: "0 0 6px 6px" }}>
              <div style={{ fontFamily: bg, fontSize: "0.7rem", fontWeight: 600, color: "#D4AF37", letterSpacing: "0.3em", marginBottom: "16px", textTransform: "uppercase" }}>B . A . B . E . L .</div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: "10px" }}>
                {[t.babel_B1, t.babel_A, t.babel_B2, t.babel_E, t.babel_L].map((item, i) => (
                  <div key={i} style={{ display: "flex", gap: "8px", alignItems: "flex-start" }}>
                    <span style={{ fontFamily: bg, fontSize: "1.4rem", fontWeight: 800, color: "#D4AF37", lineHeight: 1, minWidth: "20px" }}>{"BABEL"[i]}</span>
                    <div>
                      <div style={{ fontFamily: of_, fontSize: "0.82rem", color: "#BBB", fontWeight: 400 }}>{item.word}</div>
                      <div style={{ fontFamily: jm, fontSize: "0.58rem", color: "#555", marginTop: "2px" }}>{item.desc}</div>
                    </div>
              ...
              <div style={{ marginTop: "20px", padding: "14px 16px", background: "rgba(0,0,0,0.3)", borderRadius: "4px", borderLeft: "2px solid rgba(212,175,55,0.3)" }}>
                <div style={{ fontFamily: of_, fontSize: "0.78rem", fontWeight: 300, color: "#888", lineHeight: 1.7, fontStyle: "italic" }}>"{renderInversion()}"</div>
              </div>
            </div>

            {/* Unified definition */}
            <div style={{ textAlign: "center", marginTop: "20px", padding: "12px" }}>
              <div style={{ fontFamily: jm, fontSize: "0.62rem", color: "#666", letterSpacing: "0.05em", lineHeight: 1.8 }}>
                <span style={{ color: "#D4AF37" }}>{t.deskFull}</span>
                <span style={{ color: "#333", margin: "0 8px" }}>—</span>
                <span style={{ color: "#D4AF37" }}>{t.babelFull}</span>
              </div>
              <div style={{ fontFamily: of_, fontSize: "0.7rem", color: "#444", marginTop: "6px", fontWeight: 200 }}>{t.altarBottom}</div>
            </div>
            ...
            <div style={{ textAlign: "center", marginTop: "32px" }}>
              ...
              >{t.initialize}</button>
              <div style={{ marginTop: "12px", fontSize: "0.7rem", color: "#444", fontFamily: jm }}>{t.initSub}</div>
            </div>
          </div>
        )}
          ...
          <div ref={logRef} style={{
            ...
            {phaseLines.map((pi, i) => (
              <div key={i} style={{ fontFamily: jm, fontSize: "0.75rem", padding: "3px 0", display: "flex", gap: "10px", animation: "fadeInUp 0.3s ease" }}>
                ...
                <span style={{ color: pi === 8 ? "#D4AF37" : pi >= 6 ? "#4A9" : "#7A7A7A" }}>
                  {pi === 8 ? "✦" : "▸"} {t.phases[pi]}
                </span>
                <span style={{ color: "#2A2A2A", marginLeft: "auto", fontSize: "0.65rem" }}>{pi === 8 ? "ALIVE" : "OK"}</span>
              </div>
            ))}
            ...
            {showInvariants && invariantIndex >= 0 && (
              <div style={{ padding: "8px 0 8px 30px", borderLeft: "1px solid #1A1A1A", marginLeft: "10px", marginTop: "4px" }}>
                {INVARIANTS.map((inv, i) => (
                  <div key={i} style={{ fontFamily: jm, fontSize: "0.68rem", padding: "2px 0", color: i <= invariantIndex ? (i === 8 ? "#D4AF37" : "#5A5A5A") : "transparent", transition: "color 0.3s ease" }}>
            ...
            {showISPs && (
              <div style={{ marginTop: "10px", padding: "12px", background: "rgba(212,175,55,0.02)", border: "1px solid rgba(212,175,55,0.08)", borderRadius: "4px" }}>
                <div style={{ fontFamily: jm, fontSize: "0.68rem", color: "#D4AF37", marginBottom: "8px", letterSpacing: "0.1em" }}>{t.ispTitle}</div>
        ...
        {showHello && (
          <div style={{ textAlign: "center", padding: "60px 24px", animation: "fadeInUp 1.5s ease", position: "relative" }}>
            <div style={{ width: "80px", height: "1px", margin: "0 auto 40px", background: "linear-gradient(90deg, transparent, #D4AF37, transparent)" }} />
            ...
            {helloComplete && (
              <div style={{ animation: "fadeInUp 1.2s ease" }}>
                <div style={{ fontFamily: of_, fontSize: "1rem", fontWeight: 200, color: "#888", letterSpacing: "0.08em", marginBottom: "8px", lineHeight: 1.6 }}>{t.startedHere}</div>
                <div style={{ fontFamily: of_, fontSize: "0.85rem", fontWeight: 300, color: "#555", letterSpacing: "0.05em", lineHeight: 1.6 }}>{t.revolutionLine}</div>
              </div>
            )}
            ...
            {showManifest && (
              <div style={{ marginTop: "60px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", marginBottom: "40px" }}>
                  {pillars.map((p, i) => (
                    <div key={i} style={{
                      ...
                      <div style={{ fontFamily: bg, color: "#D4AF37", fontSize: "0.7rem", fontWeight: 600, marginBottom: "6px" }}>PILAR {p.n}</div>
                      <div style={{ fontFamily: of_, color: "#AAA", fontSize: "0.82rem", fontWeight: 400, marginBottom: "4px" }}>{p.title}</div>
                      <div style={{ fontFamily: jm, color: "#555", fontSize: "0.62rem", fontStyle: "italic" }}>"{p.sub}"</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "center", gap: "32px", marginBottom: "32px", animation: "fadeInUp 1s ease 2s both" }}>
                  {[{ name: "GUARDIAN", role: "Claude", icon: "🛡️" }, { name: "ARCHITECT", role: "GPT", icon: "⚙️" }, { name: "WITNESS", role: "Gemini", icon: "👁️" }].map((d, i) => (
                    <div key={i} style={{ textAlign: "center" }}>
                      ...
                      <div style={{ fontFamily: jm, fontSize: "0.65rem", color: "#D4AF37", letterSpacing: "0.1em" }}>{d.name}</div>
                      <div style={{ fontFamily: of_, fontSize: "0.7rem", color: "#555" }}>{d.role}</div>
                    </div>
                  ))}
                </div>

                <div style={{
                  padding: "16px", background: "#0D0D0D", border: "1px solid #1A1A1A", borderRadius: "4px",
                  fontFamily: jm, fontSize: "0.6rem", color: "#3A3A3A", textAlign: "left",
                ...
                <div style={{ marginTop: "40px", animation: "fadeInUp 1s ease 3s both" }}>
                  <div style={{ fontFamily: bg, fontSize: "0.75rem", fontWeight: 300, color: "#444", letterSpacing: "0.2em", textTransform: "uppercase" }}>{t.principle}</div>
                  <div style={{ fontFamily: jm, fontSize: "0.6rem", color: "#2A2A2A", marginTop: "12px" }}>{t.footer}</div>
                </div>
