...
"""
╔══════════════════════════════════════════════════════════════╗
║           WINDI GOVERNANCE GUARD AGENT v1.1                  ║
║                                                              ║
...
WINDI_PORTS = {
    ...
    "warroom_dispatch":  {"port": 8090, "path": "/health",        "critical": True},
...
class HealthProbe:
    ...
    def check_service(self, name: str, config: Dict) -> HealthCheckResult:
        ...
        try:
            ...
            req.add_header("User-Agent", "WINDI-GovernanceGuard/1.1")
...
class ChainWatcher:
    ...
    def verify_virtue_chain(self) -> Dict:
        ...
        if result["breaks_found"] > 0:
            ...
            log.info(
                "ChainWatcher: %d records verified, 0 breaks. Chain INTACT.",
...
class ISPScanner:
    ...
    # Flexible field mapping — accepts multiple field name variants
    FIELD_VARIANTS = {
        "profile_id":       ["profile_id", "id", "profileId", "slug"],
        "profile_name":     ["profile_name", "name", "profileName", "display_name", "title"],
        "version":          ["version", "ver", "schema_version", "isp_version"],
        "governance_level": ["governance_level", "level", "gov_level", "governanceLevel", "governance"],
        "institution":      ["institution", "org", "organization", "company", "entity", "institution_name"],
    }

    # Which fields are strictly required vs recommended
    STRICTLY_REQUIRED = ["profile_id", "profile_name"]
    RECOMMENDED = ["version", "governance_level", "institution"]

    GOVERNANCE_LEVELS = ["L1", "L2", "L3", "l1", "l2", "l3"]
    ...
    def _find_field(self, data: Dict, field_key: str) -> Any:
        """Find a field value using flexible variant matching."""
        variants = self.FIELD_VARIANTS.get(field_key, [field_key])
        ...
        for variant in variants:
            if variant.lower() in data_lower:
                return data_lower[variant.lower()]
        return None
    ...
    def _validate_profile(self, profile_dir: Path) -> ISPCheckResult:
        ...
        if governance_level is not None:
            gov_str = str(governance_level).upper()
            if not gov_str.startswith("L"):
                gov_str = f"L{gov_str}"
            if gov_str not in ["L1", "L2", "L3"]:
                issues.append(f"Invalid governance_level: {governance_level}")
            else:
                governance_level = gov_str

        # Extract identity license (flexible — could be string, dict, or nested)
        identity = (
            data.get("identity_license") or
        ...
        if not has_templates:
            # Check for inline templates in profile data
            if any(k for k in data.keys() if "template" in k.lower()):
                has_templates = True
            # Check for sections/blocks that serve as templates
            if any(k for k in data.keys() if k in ("sections", "blocks", "layout", "structure", "header", "footer")):
                has_templates = True
        if not has_templates:
...
<body>
    <div class="container">
        ...
        <div class="footer">
            <div>WINDI Governance Guard Agent v1.1 — Three Dragons Protocol</div>
...
class GovernanceGuard:
    ...
    def run_once(self):
        ...
        if results["chain"]:
            ...
            print(f"\n🔗 ChainWatcher: {icon} {chain['records_checked']} records, {chain['breaks_found']} breaks")
        ...
        if results["isp"]:
            ...
            for r in results["isp"]:
                ...
                print(f"   {icon} {r.profile_id} [L{r.governance_level or '?'}]{issues_str}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="WINDI Governance Guard Agent v1.1",
