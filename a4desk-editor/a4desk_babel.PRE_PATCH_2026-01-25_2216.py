...
box.innerHTML+='<div class="llm-msg llm-user">'+msg+'</div>';
input.value='';
box.innerHTML+='<div class="llm-msg llm-windi">... Thinking...</div>';
box.scrollTop=box.scrollHeight;
try{
const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:msg,context:editorText,dragon:'claude'})});
const d=await r.json();
box.lastChild.remove();
if(d.response){
const respId='resp-'+Date.now();
box.innerHTML+='<div class="llm-msg llm-windi" id="'+respId+'"><pre class="llm-text">'+d.response+'</pre><div class="llm-receipt">'+d.receipt+'</div><button class="btn-insert" data-rid="'+respId+'" onclick="insertToEditor(this.dataset.rid)">+ Insert</button></div>';
}else{
box.innerHTML+='<div class="llm-msg llm-windi">[!] '+d.error+'</div>';
}
}catch(e){box.lastChild.innerHTML='[!] Error: '+e;}
box.scrollTop=box.scrollHeight;}
function insertToEditor(respId){
const resp=document.getElementById(respId);
if(!resp)return;
const text=resp.innerText.replace(/+ Insert$/,'').replace(/WINDI-.*$/,'').trim();
