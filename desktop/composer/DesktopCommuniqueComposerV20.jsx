...
// ═══════════════════════════════════════════════════════════════
//  WINDI Desktop Communiqué Composer v2.0
//  "AI processes. Human decides. WINDI guarantees."
//
//  v2.0 — ISP Template Integration (Guardian + Architect)
//  ✔ 6 ISP Communiqué Templates (Instruments of Governance)
//  ✔ Auto-fill: impact, risk, authority on template select
//  ✔ isp_governance block in manifest payload
//  ✔ Accent color + authority badge on template cards
//  ✔ Auto-fill indicator with governance preview
//
...
// ═══════════════════════════════════════════════════════════════
//  ISP COMMUNIQUÉ TEMPLATES — 6 Instruments of Governance
//  Each template is a "state signature" with visual DNA + governance rules.
//  "O template NUNCA decide o nível. A API decide. O template apenas manifesta."
// ═══════════════════════════════════════════════════════════════
const ISP_COM_TEMPLATES = [
  {
    id: "ISP-COM-01", icon: "\u269C", codename: "The Sovereign",
    accent: "#D4AF37", theme: "noir",
    authority: "EXECUTIVE", defaultImpact: "CRIT", defaultRisk: "R5",
    label: { de: "WINDI Official", en: "WINDI Official", pt: "WINDI Oficial" },
    desc: { de: "H\u00F6chste Autorisierungsstufe", en: "Highest authorization", pt: "Autoriza\u00E7\u00E3o m\u00E1xima" },
  },
  {
    id: "ISP-COM-02", icon: "\uD83D\uDCF0", codename: "The Newsroom",
    accent: "#1A1A1A", theme: "klar",
    authority: "STANDARD", defaultImpact: "MED", defaultRisk: "R2",
    label: { de: "Jornaline Modern", en: "Jornaline Modern", pt: "Jornaline Moderno" },
    desc: { de: "Journalistisches Format", en: "Journalistic format", pt: "Formato jornal\u00EDstico" },
  },
  {
    id: "ISP-COM-03", icon: "\uD83D\uDEA8", codename: "The Guardian",
    accent: "#EF4444", theme: "noir",
    authority: "ELEVATED", defaultImpact: "CRIT", defaultRisk: "R4",
    label: { de: "Sicherheitswarnung", en: "Security Advisory", pt: "Alerta de Seguran\u00E7a" },
    desc: { de: "Systemalarme und Widerruf", en: "System alerts & revocations", pt: "Alertas e revoga\u00E7\u00F5es" },
  },
  {
    id: "ISP-COM-04", icon: "\uD83D\uDCDC", codename: "The Law",
    accent: "#F97316", theme: "noir",
    authority: "LEGISLATIVE", defaultImpact: "HIGH", defaultRisk: "R3",
    label: { de: "Governance-Erlass", en: "Governance Decree", pt: "Decreto de Governan\u00E7a" },
    desc: { de: "Sentinel LAW Regelwerke", en: "Sentinel LAW rules", pt: "Regras Sentinel LAW" },
  },
  {
    id: "ISP-COM-05", icon: "\uD83D\uDCCA", codename: "The Transparency",
    accent: "#3B82F6", theme: "klar",
    authority: "STANDARD", defaultImpact: "MED", defaultRisk: "R2",
    label: { de: "Statusbericht", en: "Status Report", pt: "Relat\u00F3rio de Status" },
    desc: { de: "Metriken und Transparenz", en: "Metrics & transparency", pt: "M\u00E9tricas e transpar\u00EAncia" },
  },
  {
    id: "ISP-COM-06", icon: "\uD83D\uDCDD", codename: "The Confidential",
    accent: "#94a3b8", theme: "noir",
    authority: "INTERNAL", defaultImpact: "LOW", defaultRisk: "R1",
    label: { de: "Interne Mitteilung", en: "Internal Memo", pt: "Memorando Interno" },
    desc: { de: "Vertrauliche Kommunikation", en: "Confidential comm.", pt: "Comunica\u00E7\u00E3o confidencial" },
  },
];

// Authority level color mapping
const AUTHORITY_COLORS = {
  EXECUTIVE: "#D4AF37", ELEVATED: "#EF4444", LEGISLATIVE: "#F97316",
  STANDARD: "#3B82F6", INTERNAL: "#94a3b8",
};
...
const i18n = {
  ...
  authority: { de: "Autorisierung", en: "Authority", pt: "Autorização" },
  autoFill: { de: "Auto-Governance", en: "Auto-Governance", pt: "Auto-Governança" },
  templateNote: { de: "Template bestimmt Governance-Vorgaben", en: "Template sets governance defaults", pt: "Template define governança padrão" },
};
...
export default function DesktopCommuniqueComposer() {
  ...
  const [template, setTemplate] = useState("ISP-COM-02");
  ...
  // ═══════════════════════════════════════════════════════════
  //  ISP TEMPLATE SELECTION — Auto-fill governance fields
  // ═══════════════════════════════════════════════════════════
  const handleTemplateSelect = useCallback((templateId) => {
    setTemplate(templateId);
    const tmpl = ISP_COM_TEMPLATES.find(t => t.id === templateId);
    if (tmpl) {
      setImpactLevel(tmpl.defaultImpact);
      setRiskLevel(tmpl.defaultRisk);
      setAuthorityLevel(tmpl.authority);
    }
  }, []);

  // ═══════════════════════════════════════════════════════════
  //  BUILD PAYLOAD — v2.0 with isp_governance block
  // ═══════════════════════════════════════════════════════════
  const buildPayload = useCallback(() => {
    const tmpl = ISP_COM_TEMPLATES.find(tp => tp.id === template);
    return {
      ...
      isp_governance: {
        template_id: template,
        template_name: tmpl?.label?.en || template,
        authority_level: authorityLevel,
        impact_level: impactLevel,
        risk_level: riskLevel,
        theme: tmpl?.theme || "noir",
      },
  ...
  }, [docTitle, author, template, authorityLevel, blocks, impactLevel, department, docLang, tags, riskLevel, mediaFiles]);
  ...
  const s = {
    ...
    tplCard: (on, accent) => ({ display: "flex", alignItems: "center", gap: 9, padding: "8px 10px", borderRadius: 7, cursor: "pointer", transition: "all 0.12s", backgroundColor: on ? `${accent}0F` : "transparent", border: `1px solid ${on ? `${accent}40` : "transparent"}`, position: "relative", overflow: "hidden" }),
    tplAccent: (accent, on) => ({ position: "absolute", left: 0, top: 0, bottom: 0, width: on ? 3 : 0, backgroundColor: accent, transition: "width 0.15s", borderRadius: "7px 0 0 7px" }),
    tplIcon: (on, accent) => ({ fontSize: 15, color: on ? accent : C.textDim, flexShrink: 0 }),
    tplLabel: (on, accent) => ({ fontSize: 11, fontWeight: 600, color: on ? accent : C.text }),
    tplDesc: { fontSize: 9, color: C.textDim, marginTop: 1 },
    tplAuthority: (color) => ({ display: "inline-block", fontSize: 7, fontFamily: font.mono, fontWeight: 600, padding: "1px 5px", borderRadius: 3, backgroundColor: `${color}18`, color, letterSpacing: "0.04em", textTransform: "uppercase", marginTop: 3 }),
    autoFillBox: { display: "flex", flexWrap: "wrap", gap: 5, padding: "7px 9px", borderRadius: 6, backgroundColor: C.successDim, border: `1px solid rgba(34,197,94,0.15)`, marginTop: 6 },
    autoFillTag: (color) => ({ fontSize: 8, fontFamily: font.mono, fontWeight: 600, padding: "2px 6px", borderRadius: 3, backgroundColor: `${color}16`, color }),
        ...
        <div style={s.side}>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {ISP_COM_TEMPLATES.map(tpl => {
                const on = template === tpl.id;
                const ac = tpl.accent;
                return (
                  <div key={tpl.id} style={s.tplCard(on, ac)} onClick={() => handleTemplateSelect(tpl.id)}>
                    <div style={s.tplAccent(ac, on)} />
                    <span style={s.tplIcon(on, ac)}>{tpl.icon}</span>
                    <div style={{ paddingLeft: on ? 2 : 0, transition: "padding 0.12s" }}>
                      <div style={s.tplLabel(on, ac)}>{tpl.label[lang]}</div>
                      <div style={s.tplDesc}>{tpl.desc[lang]}</div>
                      <div style={s.tplAuthority(AUTHORITY_COLORS[tpl.authority] || C.textDim)}>
                        {tpl.authority}
                      </div>
                    </div>
                  </div>
            ...
            {/* Auto-fill indicator */}
            {(() => {
              const cur = ISP_COM_TEMPLATES.find(tp => tp.id === template);
              if (!cur) return null;
              return (
                <div style={s.autoFillBox}>
                  <span style={{ fontSize: 8, color: C.success, fontFamily: font.mono, width: "100%" }}>
                    {t("autoFill", lang)}:
                  </span>
                  <span style={s.autoFillTag(cur.accent)}>Impact: {cur.defaultImpact}</span>
                  <span style={s.autoFillTag(RISK_COLORS[cur.defaultRisk] || "#888")}>Risk: {cur.defaultRisk}</span>
                  <span style={s.autoFillTag(AUTHORITY_COLORS[cur.authority] || "#888")}>{cur.authority}</span>
                </div>
              );
            })()}
          </div>
        ...
        <div style={s.right}>
          ...
          <div>
            ...
            {[
              ...
              ["Template", ISP_COM_TEMPLATES.find(tp => tp.id === template)?.label[lang]],
              [t("blocks", lang), `${blocks.length}`],
              ["Impact", impactLevel],
              ["Risk", riskLevel],
              [t("authority", lang), authorityLevel],
