# WINDI-TRAVEL · IDENTITY GATE — Master Prompt para o Gêmeo
...
O WINDI-TRAVEL é um galho soberano da ONE TREE (windi-domain.com).
...
Construir o **WINDI-TRAVEL Identity Gate** — o nascimento jurídico
...
│   └── windi-travel.service      ← systemd service (NOVO)
├── landing/
│   └── index.html             ← windi-travel-landing-v1.html (JÁ EXISTE)
└── workspace/
    └── index.html             ← windi-travel-workspace-v3.html (JÁ EXISTE)
```

### 0.2 Porta dedicada
```
Port: :8120  ← WINDI-TRAVEL Identity Gate
```
...
location /law/workspace/ {
    alias /opt/windi/windi-travel/workspace/;
    index index.html;
}

location /law/landing/ {
    alias /opt/windi/windi-travel/landing/;
    index index.html;
}
```
...
### Stack
- FastAPI + SQLite (windi_travel_identity.db)
...
### Schema SQLite — windi_travel_identity.db
...
async def seal_identity_in_ledger(company_id, did, admin_email):
    receipt_id = f"WINDI-TRAVEL-IDENTITY-{company_id[:8].upper()}"
    payload = {
        "id": receipt_id,
        "actor": admin_email,
...
<div class="identity-gate-wall">
  ...
  <div class="igw-sub">
    O WINDI-TRAVEL requer uma identidade verificável antes de qualquer acção.
...
```ini
# /etc/systemd/system/windi-travel.service
[Unit]
...
[Service]
User=windi
WorkingDirectory=/opt/windi/windi-travel/identity-gate
ExecStart=/usr/bin/python3 -m uvicorn identity_gate:app --host 127.0.0.1 --port 8120
Restart=always
RestartSec=5
EnvironmentFile=/opt/windi/.env
StandardOutput=append:/opt/windi/logs/windi-travel.log
StandardError=append:/opt/windi/logs/windi-travel-error.log
...
sudo systemctl status windi-travel
```
...
curl -X POST https://windi-domain.com/ledger/api/receipts \
  ...
  -d '{
    "id": "WINDI-TRAVEL-IDENTITY-GATE-V1.0.0",
    "actor": "Jober Mögele Correa",
    "app": "windi-travel",
    "doc_name": "WINDI-TRAVEL Identity Gate v1.0.0 — Architecture Seal",
    "doc_type": "doc",
    "governance_level": "HIGH",
    "metadata": {
      "version": "v1.0.0",
...
curl -s https://windi-domain.com/ledger/api/receipts/WINDI-TRAVEL-IDENTITY-GATE-V1.0.0 \
  | python3 -m json.tool
```
...
peça que falta para o WINDI-TRAVEL passar de demo para sistema real.

Sequência de execução:
1. Criar directório /opt/windi/windi-travel/
2. Implementar identity_gate.py (FastAPI :8120)
3. Construir gate.html (wizard 5 blocos, NOIR/KLAR, trilingue)
4. Criar windi-travel.service (systemd)
