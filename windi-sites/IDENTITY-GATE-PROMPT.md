# WINDI-SITES · IDENTITY GATE — Master Prompt para o Gêmeo
...
O WINDI-SITES é um galho soberano da ONE TREE (windi-domain.com).
...
Construir o **WINDI-SITES Identity Gate** — o nascimento jurídico
...
```
/opt/windi/windi-sites/
├── identity-gate/
│   ├── identity_gate.py       ← FastAPI app (NOVO)
│   ├── templates/
│   │   └── gate.html          ← UI do Identity Gate (NOVO)
│   └── windi-sites.service      ← systemd service (NOVO)
├── landing/
│   └── index.html             ← windi-sites-landing-v1.html (JÁ EXISTE)
└── workspace/
    └── index.html             ← windi-sites-workspace-v3.html (JÁ EXISTE)
```

### 0.2 Porta dedicada
```
Port: :8120  ← WINDI-SITES Identity Gate
```
...
location /law/workspace/ {
    alias /opt/windi/windi-sites/workspace/;
    index index.html;
}

location /law/landing/ {
    alias /opt/windi/windi-sites/landing/;
    index index.html;
}
```
...
### Stack
- FastAPI + SQLite (windi_sites_identity.db)
...
### Schema SQLite — windi_sites_identity.db
...
async def seal_identity_in_ledger(company_id, did, admin_email):
    receipt_id = f"WINDI-SITES-IDENTITY-{company_id[:8].upper()}"
    payload = {
        "id": receipt_id,
        "actor": admin_email,
...
<div class="identity-gate-wall">
  ...
  <div class="igw-sub">
    O WINDI-SITES requer uma identidade verificável antes de qualquer acção.
...
```ini
# /etc/systemd/system/windi-sites.service
[Unit]
...
[Service]
User=windi
WorkingDirectory=/opt/windi/windi-sites/identity-gate
ExecStart=/usr/bin/python3 -m uvicorn identity_gate:app --host 127.0.0.1 --port 8120
Restart=always
RestartSec=5
EnvironmentFile=/opt/windi/.env
StandardOutput=append:/opt/windi/logs/windi-sites.log
StandardError=append:/opt/windi/logs/windi-sites-error.log
...
sudo systemctl enable windi-sites
sudo systemctl start windi-sites
sudo systemctl status windi-sites
```
...
curl -X POST https://windi-domain.com/ledger/api/receipts \
  ...
  -d '{
    "id": "WINDI-SITES-IDENTITY-GATE-V1.0.0",
    "actor": "Jober Mögele Correa",
    "app": "windi-sites",
    "doc_name": "WINDI-SITES Identity Gate v1.0.0 — Architecture Seal",
    "doc_type": "doc",
    "governance_level": "HIGH",
    "metadata": {
      "version": "v1.0.0",
...
curl -s https://windi-domain.com/ledger/api/receipts/WINDI-SITES-IDENTITY-GATE-V1.0.0 \
  | python3 -m json.tool
```
...
peça que falta para o WINDI-SITES passar de demo para sistema real.

Sequência de execução:
1. Criar directório /opt/windi/windi-sites/
2. Implementar identity_gate.py (FastAPI :8120)
3. Construir gate.html (wizard 5 blocos, NOIR/KLAR, trilingue)
4. Criar windi-sites.service (systemd)
