"""
WINDI-SITES Identity Gate v1.0.0
Port: :8192
Sovereign Identity Management for Site Clients

§4 Identity Gate — 3 DID levels:
  - DID do Cliente: NODAL (2) for external, SOVEREIGN/ORACLE for verticals
  - DID do Site: SEED (1) at C6 seal
  - DID do Visitante: SEED (1) on verified action
...
try:
    ...
    load_dotenv("/opt/windi/windi-sites/identity-gate/.env")
except ImportError:
...
ADMIN_SECRET = os.environ.get("WINDI_ADMIN_SECRET", "windi-sites-admin-2026")
...
# ═══════════════════════════════════════════════════════════════
# WINDI VERTICAL DIDs (§4 Identity Gate — Fork for Internals)
# When client is a WINDI vertical, reuse existing DID (SOVEREIGN/ORACLE)
# ═══════════════════════════════════════════════════════════════

WINDI_VERTICAL_DIDS = {
    # WINDI-LAW — SOVEREIGN tier
    "did:windi:law-001": {"tier": "SOVEREIGN", "name": "WINDI-LAW", "port": 8122},
    # WINDI-TRAVEL — SOVEREIGN tier
    "did:windi:travel-001": {"tier": "SOVEREIGN", "name": "WINDI-TRAVEL", "port": 8126},
    # WINDI-ENTERPRISE — ORACLE tier
    "did:windi:enterprise-001": {"tier": "ORACLE", "name": "WINDI-ENTERPRISE", "port": 8150},
    # Human Dragon — ORACLE tier (root authority)
    "did:windi:dragon-001": {"tier": "ORACLE", "name": "HUMAN-DRAGON", "port": None},
}


def is_windi_vertical(did: str) -> bool:
    """Check if DID belongs to a WINDI vertical."""
    return did in WINDI_VERTICAL_DIDS


def get_vertical_info(did: str) -> Optional[dict]:
    """Get vertical info for a WINDI internal DID."""
    return WINDI_VERTICAL_DIDS.get(did)


# ═══════════════════════════════════════════════════════════════
# HOST-AWARE ROUTING (windisites.de vs windi-domain.com/sites/)
# ═══════════════════════════════════════════════════════════════

def get_base_path(request: Request) -> str:
    """
    Returns '' for windisites.de, '/sites' for windi-domain.com
    X-Forwarded-Host has priority (nginx proxy)
    """
    host = request.headers.get("host", "")
    forwarded = request.headers.get("x-forwarded-host", "")
    effective_host = forwarded or host
    if "windisites.de" in effective_host:
        return ""
    return "/sites"
...
async def send_verification_email(email: str, token: str, full_name: str, lang: str = "en"):
    ...
    subjects = {
        "de": "WINDI-SITES: Bestätigen Sie Ihre E-Mail",
        "en": "WINDI-SITES: Confirm your email",
        "pt": "WINDI-SITES: Confirme seu email"
    }
    ...
    try:
        ...
        <body style="font-family: 'JetBrains Mono', monospace; color: #1A1A1A; max-width: 600px;">
            <div style="border-bottom: 2px solid #8B7424; padding-bottom: 10px; margin-bottom: 20px;">
                <strong style="color: #8B7424;">WINDI-SITES</strong>
            </div>
            <p>{html_body}</p>
...
async def send_login_email(email: str, token: str, full_name: str, lang: str = "en", pin: str = None):
    ...
    subjects = {
        "de": "WINDI-SITES: Dein Login-Code",
        "en": "WINDI-SITES: Your login code",
        "pt": "WINDI-SITES: Teu código de acesso"
    }
...
Hier ist dein Login-Link für WINDI-SITES:
...
Here is your login link for WINDI-SITES:
...
Aqui está o teu link de acesso ao WINDI-SITES:
    ...
    try:
        ...
        <body style="font-family: 'JetBrains Mono', monospace; color: #1A1A1A; max-width: 600px;">
            <div style="border-bottom: 2px solid #1a3a6b; padding-bottom: 10px; margin-bottom: 20px;">
                <strong style="color: #1a3a6b;">WINDI-SITES</strong>
            </div>
            <p>{html_body}</p>
            <div style="margin-top: 30px; padding-top: 10px; border-top: 1px solid #E0DED8; font-size: 12px; color: #666;">
                WINDI Publishing House · Kempten, Bavaria
...
async def seal_identity_in_ledger(company_id: str, did: str, admin_email: str, event_type: str = "GENESIS"):
    ...
    receipt_id = f"WINDI-SITES-{event_type}-{company_id[:8].upper()}-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
...
app = FastAPI(
    title="WINDI-SITES Identity Gate",
    version=VERSION,
...
try:
    ...
    print("[WINDI-SITES] W-DRAGON-001 Dragon Shadow Forest loaded")
...
try:
    ...
    print("[WINDI-SITES] W-CANVAS-001 integration loaded")
except ImportError as e:
    CANVAS_AVAILABLE = False
    print(f"[WINDI-SITES] W-CANVAS-001 not available: {e}")

# Mount static files and templates
templates = Jinja2Templates(directory="/opt/windi/windi-sites/identity-gate/templates")
...
async def startup():
    init_db()
    print(f"[WINDI-SITES] Identity Gate {VERSION} started on :8192")
...
async def health():
    ...
    return {
        "service": f"WINDI-SITES Identity Gate {VERSION}",
        "status": "healthy",
        "port": 8192,
        "companies": companies,
        "admins": admins,
...
async def register(data: CompanyRegister, request: Request):
    ...
    try:
        ...
        if "unique" in err and "email" in err:
            ...
            import logging
            logging.error(f"[WINDI-SITES] Register IntegrityError: {e}")
    ...
    <body>
        ...
        <script>
            ...
            try {{
                ...
                sessionStorage.setItem('windi_sites_wallet', JSON.stringify(walletData));
                sessionStorage.setItem('windi_sites_did', "{did}");
                sessionStorage.setItem('windi_sites_fingerprint', "{fingerprint}");
            }} catch(e) {{
                console.warn('[WINDI-SITES] sessionStorage not available:', e);
            }}
...
async def login_with_pin(data: PinLogin, request: Request):
    ...
    return {
        ...
        "state": state,
        "full_name": full_name,
        "workspace_url": "/sites/workspace/?did=" + did
    }
...
async def workspace_static(filename: str, request: Request):
    ...
    filepath = f"/opt/windi/windi-sites/workspace/{filename}"
...
async def dashboard_json(did: str):
    ...
    # Padrão: WINDI-SITES-GENESIS-{did_uuid[:8].upper()}
    did_uuid = did.split(":")[-1] if ":" in did else did
    did_short = did_uuid.replace("-", "")[:8].upper()
    genesis_receipt_id = f"WINDI-SITES-GENESIS-{did_short}"
    ...
    return {
        "windi_identity_card": {
            ...
            "issued_by": "WINDI-SITES · Liga IA+H · Kempten, Bavaria"
        }
    }
...
async def admin_verify(did: str, x_admin_secret: Optional[str] = Header(None)):
    ...
    receipt_id = f"WINDI-SITES-VERIFY-{did[10:18].upper()}-{datetime.now(timezone.utc).strftime('%Y%m%d%H%M%S')}"
    content_hash = f"sha256:{hashlib.sha256(f'{did}|VERIFIED|{now}'.encode()).hexdigest()}"

    ledger_payload = {
        "id": receipt_id,
        "actor": "Human Dragon",
        "app": "windi-sites-admin",
...
class LawCoverRequest(BaseModel):
    ...
    jurisdiction: str
    doc_type: str
    actor: str = "windi-sites"

@app.post("/api/canvas/sites-cover")
async def generate_sites_cover(body: LawCoverRequest):
    """
    Generate branded cover for site documents.
